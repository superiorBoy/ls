...
/* WEBPACK VAR INJECTION */(function (uni) {
                Object.defineProperty(exports, "__esModule", { value: true }); exports.default = void 0; var _regenerator = _interopRequireDefault(__webpack_require__(/*! ./node_modules/@babel/runtime/regenerator */ 98));
                ...
                var recorderManager = uni.getRecorderManager(); var innerAudioContext = uni.createInnerAudioContext(); innerAudioContext.autoplay = true; var emotion = function emotion() { __webpack_require__.e(/*! require.ensure | components/bkhumor-emoji/index */ "components/bkhumor-emoji/index").then((function () { return resolve(__webpack_require__(/*! @/components/bkhumor-emoji/index.vue */ 1184)); }).bind(null, __webpack_require__)).catch(__webpack_require__.oe); }; var uParse = function uParse() { Promise.all(/*! require.ensure | components/feng-parse/parse */[__webpack_require__.e("common/vendor"), __webpack_require__.e("components/feng-parse/parse")]).then((function () { return resolve(__webpack_require__(/*! @/components/feng-parse/parse.vue */ 1163)); }).bind(null, __webpack_require__)).catch(__webpack_require__.oe); }; var lfile = function lfile() { __webpack_require__.e(/*! require.ensure | components/l-file/l-file */ "components/l-file/l-file").then((function () { return resolve(__webpack_require__(/*! @/components/l-file/l-file.vue */ 1177)); }).bind(null, __webpack_require__)).catch(__webpack_require__.oe); }; var _default = {
                    computed: {
                        intIntervalTime: function intIntervalTime() {// 用于显示整数的秒数
                            console.log(Math.round(this.intervalTime)); return Math.round(this.intervalTime);
                        }
                    }, created: function created() { }, components: { emotion: emotion, uParse: uParse, lfile: lfile },
                    onLoad: function onLoad(option) {
                        var _this = this; this.ls_id = option.lawyerid; // 查看消息保留条数
                    ...
                    onShow: function onShow() {// this.$http
                        ...
                        // 		.then(res => {
                        // 		});
                    }, onReady: function onReady() { var that = this; setTimeout(function () {that.app_lianjie() }, 1000); }, onHide: function onHide() { }, onUnload: function onUnload() {uni.closeSocket();// console.log('onUnload');
                        // this.time1 = '0';
                        ...
                        var that = this; var is_cuzai = false; if (user_chat_list) {
                            user_chat_list = JSON.parse(user_chat_list); for (var i in user_chat_list) {
                                if (user_chat_list[i].lawyer.userid == that.ls_id) {
                                    if (that.message[that.message.length - 1].addtime != that.is_last_msguptime) {
                                        console.log(user_chat_list[i].uptime, '11111111'); console.log(that.is_last_msguptime); var xinxi = { lawyerid: that.ls_id, userid: that.user.userid, content: that.message[that.message.length - 1].content, msgtype: that.message[that.message.length - 1].msgtype, readnum: 0, lawyerreadnum: 0, uptime: that.message[that.message.length - 1].addtime, lawyer: { userid: that.ls_xinxi.userid, photourl: that.ls_xinxi.photourl, nickname: that.ls_xinxi.nickname } }; user_chat_list.splice(i, 1); //存在即删除
                                        ...
                                        console.log('存在', user_chat_list[i]); console.log('重新设置'); is_cuzai = true;
                                    } else {
                                        is_cuzai = true; user_chat_list[i].content = that.message[that.message.length - 1].content; user_chat_list[i].msgtype = that.message[that.message.length - 1].msgtype; user_chat_list[i].readnum = 0; uni.setStorageSync('user_chat_list', JSON.stringify(user_chat_list)); //设置缓存
                    ...
                    methods: {
                        onPageScroll: function onPageScroll(res) {
                            var that = this; if (res.scrollTop == 0) { this.is_xiala = true; if (this.is_all) { uni.showToast({ title: '没有更多内容了', duration: 2000, icon: 'none' }); uni.stopPullDownRefresh(); } else { uni.showLoading({ title: '加载中', mask: true }); this.page++; this.huoqu_xiaoxi_list(); } } var info = uni.createSelectorQuery().select('.chat_body'); info.boundingClientRect(function (data) {//data - 各种参数
                        ...
                        dian_luyin: function dian_luyin() {
                            var _this7 = this; var that = this; this.is_yuyin = true; this.timer = setInterval(function () {
                                _this7.intervalTime += 0.5; if (_this7.intervalTime >= 0.5 && !_this7.isRecord) {//如果用户录制的时间太短,就不会去开启录音, 因为有个bug: recorderManager.stop()在短时间内开启在关闭的话,实际上他还在不停地录音,不知道你们有没有遇到过
                                    console.log('开始录音'); _this7.isRecord = true; _this7.intervalTime = 0; recorderManager.start({ format: 'mp3' });
                                } // console.log(this.intervalTime)
                                if (_this7.intervalTime >= 60) { uni.showToast({ title: '不得超过60秒', duration: 2000, icon: 'none' }); that.songkai(); }
                            }, 500);
                        }, moveStop: function moveStop(e) { console.log(e.touches[0].clientY); if (e.touches[0].clientY < 650) { this.is_quxiao = true; } else { this.is_quxiao = false; } }, songkai: function songkai() {
                            var _this8 = this; var that = this; console.log('songkai', this.is_quxiao, this.intervalTime); if (this.is_quxiao) { console.log('已取消录音'); } else {
                                if (this.intervalTime <= 0.5) { console.log('录音太短了!!!'); } clearInterval(this.timer); if (this.isRecord) {
                                    setTimeout(function () {
                                        recorderManager.stop(); _this8.isRecord = false; // var data = {
                                        // 	content: '',
                                        // 	msgtype: 12,
                            ...
                            this.$http.post({ url: '/mapi/consult/chatdeatils', data: { page: this.page, userid: this.ls_id } }).then(function (res) {
                                ...
                                if (res.data.overtime.length > 0) { _this13.chaoshi_item = res.data.overtime; _this13.is_new = true; } var _loop = function _loop(key) {
                                    ...
                                    } if (res.data.message[key].msgtype == 10 || res.data.message[key].msgtype == 11 || res.data.message[key].msgtype == 12) { _this13.$http.post({ url: '/mapi/consult/red_envelope', data: { redid: res.data.message[key].content, userid: _this13.ls_id } }).then(function (ress) { console.log(ress); res.data.message[key] = Object.assign(res.data.message[key], ress.data.red_envelope); _this13.$forceUpdate(); }); } // 		if (res.data.message[key].msgtype == 13) {
                                    // 			console.log(key)
                        ...
                        Audio2dataURL: function Audio2dataURL(path) {
                            var that = this; plus.io.resolveLocalFileSystemURL(path, function (entry) {
                                entry.file(function (file) {
                                    var reader = new plus.io.FileReader(); reader.onloadend = function (e) {
                                        console.log(e.target.result); that.up_luyin(e.target.result); // that.dataURL2Audio(e.target.result, function(entry){
                                ...
                                }, function (e) { console.log('读写出现异常: ' + e.message); });
                            });
                        }, dataURL2Audio: function dataURL2Audio(base64Str, callback) {
                            var base64Str = base64Str.replace('data:audio/amr;base64,', ''); var audioName = new Date().valueOf() + '.amr'; plus.io.requestFileSystem(plus.io.PRIVATE_DOC, function (fs) {
                                fs.root.getFile(audioName, { create: true }, function (entry) {// 获得平台绝对路径
