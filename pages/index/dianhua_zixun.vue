				...
				<view class="zixun_item" v-for="(item, index) in dianhua_list">
					<view class="zixun_item_top">
						<view class="zixun_item_left">
							<view class="zixun_item_tx" @click="go_zhuye(item.userid)"><image :src="img_url + item.photourl" mode=""></image></view>
							<view class="zixun_item_xinxi">
								<view class="hei_26" @click="go_zhuye(item.userid)">{{ item.nickname }}律师</view>

								<view class="qian_22 item_dizhi">
									<image src="@/static/img/dianhua_dizhi.png" mode=""></image>
									{{ item.province }}-{{ item.city }}-{{ item.area }}
								</view>
								<view class="hong_26 "></view>
							</view>
						</view>
...
.zixun_item {
	...
	margin-bottom: 20rpx;
	padding: 33rpx 49rpx 33rpx 30rpx;
	box-sizing: border-box;
}
.zixun_item_top,
.zixun_item_bottom {
