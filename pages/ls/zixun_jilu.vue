		...
		<view class="zi_body tab_zi_body">
			<view class="zixun_list">
				<view class="zixun_item" v-for="(item, index) in jilu_list">
					...
					<view :class="['',item.zixunstate == 4?'qian_26_bold':'hong_26_bold']">{{ item.zixunstate == 1 ? '未付款':item.zixunstate == 2?'已付款':item.zixunstate == 3?'接单中':item.zixunstate == 4?'已完成':item.zixunstate == 5?'已退款':item.zixunstate == 6?'已取消':'' }}</view>
